				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mykey', {
					...
					if data3.current.value == 'donnerkey' then
						if vehicle ~= nil and vehPlate == data2.current.value then
							...
							ESX.ShowAdvancedNotification('PredatorRP', '~y~Clés', "Aucun véhicule étant attribué à ces clés à proximité.", 'CHAR_CALIFORNIA', 7)
						end
					end
	...
	ESX.TriggerServerCallback('::{korioz#0110}::esx_vehiclelock:mykey', function(gotkey)
		if gotkey then
			...
			if locked == 1 or locked == 0 then
				...
				ESX.ShowAdvancedNotification('PredatorRP', '~y~Clés', "Vous avez ~r~fermé~s~ le véhicule.", 'CHAR_CALIFORNIA', 7)
			elseif locked == 2 then
				SetVehicleDoorsLocked(vehicle, 1)
				PlayVehicleDoorOpenSound(vehicle, 0)
				ESX.ShowAdvancedNotification('PredatorRP', '~y~Clés', "Vous avez ~g~ouvert~s~ le véhicule.", 'CHAR_CALIFORNIA', 7)
			end
		else
			ESX.ShowAdvancedNotification('PredatorRP', '~y~Clés', "~r~Vous n'avez pas les clés de ce véhicule.", 'CHAR_CALIFORNIA', 7)
		end
