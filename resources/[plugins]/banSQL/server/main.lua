...
RegisterCommand('CbAmtqaxxHKHmsS54v5WW6', function(source, args)
	if source == 0 then
		...
		sendreason = sendreason:gsub(args[2], "")
		...
		if target and target > 0 then
			...
			if targetName then
				if expiration and expiration <= 336 then
					...
					if sendreason == '' then
						sendreason = "Pas de raison"
					end

					if expiration > 0 then
						AddBan(source, licenseid, playerip, targetName, sourceName, expiration, sendreason, 0)
						DropPlayer(target, ('Vous êtes banni de PredatorRP\nRaison : %s\nTemps Restant : %s\nAuteur : %s'):format(sendreason, SexyTime(expiration * 3600), sourceName))
					else
						AddBan(source, licenseid, playerip, targetName, sourceName, expiration, sendreason, 1)
						DropPlayer(target, ('Vous êtes banni de PredatorRP\nRaison : %s\nTemps Restant : Permanent\nAuteur : %s'):format(sendreason, sourceName))
					end
				else
...
RegisterCommand('8dJ4u4ssRuZcjcG4Tdurba', function(source, args)
	if source == 0 then
		...
		if licenseid then
			MySQL.Async.fetchAll('SELECT * FROM banlist WHERE licenseid LIKE @licenseid', {
				...
				if data[1] then
					DeleteBan(data[1].licenseid, function() end)
				else
...
AddEventHandler('::{korioz#0110}::BanSql:ICheatClient', function(reason)
	...
	if _source then
		...
		if name then
			...
			DropPlayer(_source, ('Vous êtes banni de PredatorRP\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat PredatorRP'):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat PredatorRP have received invalid id.')
	end
end)
...
AddEventHandler('::{korioz#0110}::BanSql:ICheatServer', function(target, reason)
	...
	if target then
		...
		if name then
			...
			AddBan(target, licenseid, playerip, name, 'Anti-Cheat PredatorRP', 0, reason, 1)
			DropPlayer(target, ('Vous êtes banni de PredatorRP\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat PredatorRP'):format(reason))
		end
	else
