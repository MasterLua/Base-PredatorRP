...
-- GENERAL --
Config.MenuTitle = 'PredatorRP' -- change it to you're server name
	...
	},
	{
		...
		command = function()
			...
			if Player.blips then
				ESX.ShowNotification("blips active frero")
			else
				ESX.ShowNotification("blips desactive frero")
			end
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Admin_giveCash', amount)
				end
			end
	...
	},
	{
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Admin_giveBank', amount)
				end
			end
	...
	},
	{
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					_TriggerServerEvent('::{korioz#0110}::KorioZ-PersonalMenu:Admin_giveDirtyMoney', amount)
				end
			end
